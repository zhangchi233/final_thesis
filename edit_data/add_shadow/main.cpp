...
void addRandomShadow(const std::string &image_path, const std::string &output_path) {
    ...
    for (int y = 0; y < height; ++y) {
        for (int x = 0; x < width; ++x) {
            ...
            shadow.at<uchar>(y, x) = static_cast<uchar>(255 * (1 - intensity * distance)*0.5+0);
        }
    }
    ...
    cv::addWeighted(image, 0.5, shadowColor, 0.5, 0.0, shadowedImage); // You can adjust the weight for different effects
