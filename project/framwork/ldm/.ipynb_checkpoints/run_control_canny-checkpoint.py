...
from framwork.ldm.lightning_control_canny import PanoOutpaintGenerator 
...
class CallbackLogger(Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        trainer.logger.experiment.add_images("dark_imgs", dark_imgs, global_step=self.count)
        trainer.logger.experiment.add_images("gt_imgs", gt_imgs, global_step=self.count)
        trainer.logger.experiment.add_images("pred_imgs", pred_imgs, global_step=self.count)
        trainer.logger.experiment.add_images("mask", condition, global_step=self.count)
        print("add images")
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets_dtu, batch_size=4, shuffle=True, num_workers=0,
      )
    ...
    trainer = pl.Trainer(
            **trainer_config,
            ...
            resume_from_checkpoint="/root/autodl-tmp/project/checkpoints/ldm_control.ckpt"
