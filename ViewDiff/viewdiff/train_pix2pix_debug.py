...
def train_and_test(
    ...
    # Load scheduler, tokenizer and models. (original 2d)
    ...
    for epoch in range(first_epoch, finetune_config.training.num_train_epochs):
        ...
        for step, batch in enumerate(dataloader_to_use):
            ...
            # do training step
            # save batch target and input images 
...
def update_model(finetune_config: FinetuneConfig, unet: UNet2DConditionCrossFrameInExistingAttnModel):
...
def train_step(
    ...
    if global_step%50 == 0:
        ...
        writer.add_images('input', batch['images'][0], global_step)
        writer.add_images('target', batch['target_imgs'][0], global_step)
    def process_batch(batch):
        ...
        if "images" in batch:
            ...
            print((batch["images"] - batch["target_imgs"]).abs().mean())
        ...
        # check if some timesteps within the batch is the same
        ...
        print(unet_pred.shape,unet_pred_target.shape,batch["small_mask"].shape)
