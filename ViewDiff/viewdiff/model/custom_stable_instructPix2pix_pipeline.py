...
class CustomStableDiffusionPipelineOutput(BaseOutput):
    """
    Output class for StableDiffusionInpaint3DPipeline.

    Args:
        images (`List[List[PIL.Image.Image]]` or `np.ndarray`)
            List of list of denoised PIL images of length `(batch_size, K)` or numpy array of shape `(batch_size, K, height, width,
    ...
    image_list: List[torch.FloatTensor] = None
    cached_vol_rend_grids: Optional[List[List[Dict[str, torch.FloatTensor]]]] = None


class CustomInstructPix2pixDiffusionPipeline(
    ...
    def __call__(
        ...
        decode_all_timesteps: bool = False,
        n_images_per_batch: int = -1,
    ):
        r"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                rendered_mask_per_layer_list.append(output.rendered_mask)

                if decode_all_timesteps:
                    current_image = self.vae.decode(latents / self.vae.config.scaling_factor, return_dict=False)[0]
                    current_image = self.image_processor.postprocess(
                        current_image, output_type=output_type, do_denormalize=[True] * current_image.shape[0]
                    )
                    if isinstance(current_image, torch.Tensor):
                        # never save to gpu because we always save this to file later anyways
        ...
        return CustomStableDiffusionPipelineOutput(
            ...
            rendered_mask=rendered_mask_per_layer_list,
            image_list=image_list,
            cached_vol_rend_grids=cached_vol_rend_grids
        )
