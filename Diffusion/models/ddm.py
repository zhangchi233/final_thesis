...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if self.config.model.feature_extract:
                input_high_feature = input_high1.chunk(3,dim = 0)
                input_high_feature = torch.cat(input_high_feature,dim = 1)
                data_dict["low_feature"] =  torch.cat([denoise_LL_LL,input_high_feature],dim = 1)
                input_middle_feature = input_high0.chunk(3,dim = 0)
                input_middle_feature = torch.cat(input_middle_feature,dim = 1)

                data_dict["middle_feature"] = torch.cat([pred_LL,input_middle_feature],dim = 1)
