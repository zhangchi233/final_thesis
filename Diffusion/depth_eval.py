...
from metrics import abs_error, acc_threshold
...
from run_refine import MVSSystem
# for point cloud fusion
from numba import jit
from plyfile import PlyData, PlyElement
depth_transform = T.Normalize(mean=[0.485, 0.456, 0.406],
...
def get_opts():
    ...
    parser.add_argument("--use_ori",default=False,action="store_true")
    parser.add_argument("--config",default="/root/autodl-tmp/Diffusion/configs/DTU2.yml",type=str)
    parser.add_argument("--save_pth",type= str,default="/root/autodl-tmp/Diffusion/ckpts/our_proposed/version_encoder/epoch_0.pth")
    parser.add_argument("--resume",type = str,default = "/root/autodl-tmp/model.pth.tar")
    parser.add_argument("--num_gpus",type= int,default=1)
    parser.add_argument("--light_idx",default="",type = str)
...
if __name__ == "__main__":
    ...
    proposed_model = MVSSystem(args,configs)
    proposed_model = proposed_model.to(device)
    proposed_model.prepare_data()
    proposed_model.depth_model.load_state_dict(torch.load(args.save_pth))
    dataset = proposed_model.val_dataset
    ...
    for light_idx in lights:
        if args.scan:
            data_range = [i for i, x in enumerate(dataset.metas) if x[0] == args.scan and x[1] == light_idx]
        else:
            data_range = [i for i, x in enumerate(dataset.metas) if  x[1] == light_idx]



        log_path = f'results/{args.dataset_name}/depth_{light_idx}_{args.use_ori}.log'
        log = {}
        log["our/abs_error"] = []
        log["our/acc_1"] = []
        log["our/acc_2"] = []
        log["our/acc_3"] = []
        log["theirs/abs_error"] = []
        log["theirs/acc_1"] = []
        log["theirs/acc_2"] = []
        log["theirs/acc_3"] = []
        ...
        for i in tqdm(data_range):
            ...
            depth_gt = dataset[i]["depths"]["level_0"]
            mask = dataset[i]["masks"]["level_0"]
            ...
            with torch.no_grad():
                ...
                if args.use_ori:
                    results = model(depth_transform(imgs), proj_mats, init_depth_min, depth_interval)
                    ori_results = proposed_model.predict_depth(dataset[i])
                    ...
                    batch = dataset[i]
                    results = proposed_model.predict_depth(batch)
            depth = results['depth_0'][0].cpu()
            depth = torch.nan_to_num(depth) # change nan to 0

            ori_depth = ori_results["depth_0"][0].cpu()
            ori_depth = torch.nan_to_num(ori_depth)

                    

            log["our/abs_error"].append(abs_error(depth_gt, depth, mask).mean().item())
            log["our/acc_1"].append(acc_threshold(depth_gt, depth, mask, 1).mean().item())
            log["our/acc_2"].append(acc_threshold(depth_gt, depth, mask, 2).mean().item())
            log["our/acc_3"].append(acc_threshold(depth_gt, depth, mask, 3).mean().item())
            log["theirs/abs_error"].append(abs_error(depth_gt, ori_depth, mask).mean().item())
            log["theirs/acc_1"].append(acc_threshold(depth_gt, ori_depth, mask, 1).mean().item())
            log["theirs/acc_2"].append(acc_threshold(depth_gt, ori_depth, mask, 2).mean().item())
            log["theirs/acc_3"].append(acc_threshold(depth_gt, ori_depth, mask, 3).mean().item())

            ###########################################
            print("our/abs: {:.3f} | theirs/abs: {:.3f} | ".format(np.mean(log["our/abs_error"]),np.mean(log["theirs/abs_error"])),
            "our/acc_1: {:.3f} | theirs/acc_1: {:.3f} | ".format(np.mean(log["our/acc_1"]),np.mean(log["theirs/acc_1"])),
            "our/acc_2: {:.3f} | theirs/acc_2: {:.3f} | ".format(np.mean(log["our/acc_2"]),np.mean(log["theirs/acc_2"])),
            "our/acc_3: {:.3f} | theirs/acc_3: {:.3f} | ".format(np.mean(log["our/acc_3"]),np.mean(log["theirs/acc_3"])))
            ###########################################        
        ...
        point_dir = f'results/proposed_model/points_{light_idx}_{args.use_ori}'
        ...
        for scan in scans:
            ...
            PlyData([el]).write(f'{point_dir}/{scan}_{light_idx}_{args.use_ori}.ply')
