...
from framwork.inpainting.lightning_controlnet import PanoOutpaintGenerator 
...
class CallbackLogger(Callback):
    ...
    def on_validation_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        ...
        mask = outputs["mask"].unsqueeze(1).repeat(1,3,1,1)
...
if __name__ == "__main__":
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets_dtu, batch_size=3, shuffle=True, num_workers=0,
      )
    ...
    val_loader = torch.utils.data.DataLoader(
        data_val, batch_size=2, shuffle=True, num_workers=0)
    
    lr_monitor = LearningRateMonitor(logging_interval='step')


    
    config = yaml.load(open("/root/autodl-tmp/project/configs/config_control.yaml", "r"), Loader=yaml.FullLoader)
