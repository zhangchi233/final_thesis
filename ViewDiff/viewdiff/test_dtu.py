...
class DTUDataset(Dataset):
    ...
    def  __getitem__(self, idx):
        ...
        #     target_light = np.argmin(target_light)
        light_idxs =np.random.choice(7,self.n_views)
...
class runfig:
    pretrained_model_name_or_path = "/root/autodl-tmp/output_var_unproj/all/subset_all/input_3/train/class6/saved_model_from_checkpoint-20000"
    n_input_images =3
    n_output_noise =3
...
def return_log(result1,result2,gt_depth,mask):
    ...
    return {"abs_diff":abs_diff,"abs_ratio":abs_ratio,"acc_diff1":acc_diff,"acc_ratio1":acc_ratio,
            ...
            "acc_diff4":acc_diff4,"acc_ratio4":acc_ratio4,"error1":abs_error1,"error2":abs_error2,}
...
import json
save_json = {}
writein_json = f'./results/{args.dataset_name}/log.json'
if os.path.exists(writein_json):
    with open(writein_json,'r') as f:
...
for scan in ['scan3', 'scan5', 'scan17', 'scan21', 'scan28','scan86','scan106','scan35', 'scan37','scan38', 'scan40', 'scan43', 'scan56', 'scan59', 'scan66','scan67', 'scan82', 'scan117',
             ]:
    ...
    for index,i in enumerate(data_range):
        ...
        with torch.no_grad():
            ...
            if refine == True:
                ...
                if os.path.exists(ref_path) and os.path.exists(src1_path) and os.path.exists(src2_path):
                    ...
                    image_refined = modified_imgs[0].cpu().numpy()
                    np.save(os.path.join(img_dir, f'{vid:04d}_class6.npy'), image_refined)
                    # contrast output input target

                    disply = torch.cat([imgs[0], modified_imgs[0],target_imgs[0]], dim=0)
                    save_image(disply, os.path.join(img_dir, f'{vid:04d}_class6.png'),nrow =3)
                ...
                error_modified = metric_logs["error1"].item()
                error_ori = metric_logs["error2"].item()
                error_target = metrics_2["error1"].item()

                errors1.append(error_modified)
                errors2.append(error_ori)
                errors3.append(error_target)
                
                abs_ratio.append(metric_logs["abs_ratio"].item())
                acc_ratio1.append(metric_logs["acc_ratio1"].item())
                acc_ratio2.append(metric_logs["acc_ratio2"].item())
                acc_ratio3.append(metric_logs["acc_ratio3"].item())
                acc_ratio4.append(metric_logs["acc_ratio4"].item())
                acc_diff1.append(metric_logs["acc_diff1"].item())
                acc_diff2.append(metric_logs["acc_diff2"].item())
                acc_diff3.append(metric_logs["acc_diff3"].item())
                acc_diff4.append(metric_logs["acc_diff4"].item())
                abs_diff.append(metric_logs["abs_diff"].item())
                target_diff.append(metrics_2["abs_diff"].item())
                target_ratio.append(metrics_2["abs_ratio"].item())



                
                # set description on progress bar post fix
                

                data_range.set_postfix_str(f'\r{scan} {vid:04d} '
                                f"error_modified: {error_modified},"
                                f"error_ori: {error_ori},"
                                f"error_target: {error_target},"
                                ...
                                f'acc_ratio4: {np.mean(acc_ratio4)} ',refresh=True)
                data_range.update()
                ...
                if index >=48:
                    ...
                    sys.stdout.flush()
                    save_json[scan][vid]["abs_diff"]=abs_diff
                    save_json[scan][vid]["abs_ratio"]=abs_ratio
                    save_json[scan][vid]["target_ratio"]=target_ratio
                    save_json[scan][vid]["target_diff"]=target_diff

                    save_json[scan][vid]["error_modified"] = errors1
                    save_json[scan][vid]["error_ori"] = errors2
                    save_json[scan][vid]["error_target"] = errors3
                    with open(writein_json,"w") as f:
        ...
        if args.save_visual:
            ...
            if refine:
                ...
                cv2.imwrite(os.path.join(depth_dir, f'{scan}/depth_ori_visual_{vid:04d}.jpg'),
                            depth_img)
                cv2.imwrite(os.path.join(depth_dir, f'{scan}/proba_ori_visual_{vid:04d}.jpg'),
                            (255*(proba>args.conf)).astype(np.uint8))
...
for scan in ['scan3', 'scan5', 'scan17', 'scan21', 'scan28','scan86','scan106',
             'scan35', 'scan37','scan38', 'scan40', 'scan43', 'scan56', 'scan59', 'scan66','scan67', 'scan82', 'scan117']:
            ...
            if ref_vid in image_refined: # not yet refined actually
                ...
                if read_gt:
                    ...
                    depth_ref = cv2.resize(depth_ref, tuple(args.img_wh),
                                            interpolation=cv2.INTER_LINEAR)
                    if args.save_visual:
                        depth_dir = f'./results/{args.dataset_name}/depth/{scan}'
                        mi = np.min(depth_ref[depth_ref>0])
                        ma = np.max(depth)
                        depth = (depth-mi)/(ma-mi+1e-8)
                        depth = (255*depth).astype(np.uint8)
                        depth_img = cv2.applyColorMap(depth, cv2.COLORMAP_JET)
                        cv2.imwrite(os.path.join(depth_dir, f'{scan}/depth_gt_visual_{vid:04d}.jpg'),
                            depth_img)
                        cv2.imwrite(os.path.join(depth_dir, f'{scan}/proba_gt_visual_{vid:04d}.jpg'),
                                    (255*(proba>args.conf)).astype(np.uint8))
